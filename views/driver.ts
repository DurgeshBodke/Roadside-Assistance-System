...
const SERVICE_OPTIONS: { value: ServiceType; label: string; price: number }[] = [
  { value: 'towing', label: 'Towing', price: 2500 },
  { value: 'flat-tire', label: 'Flat Tire', price: 800 },
  { value: 'fuel', label: 'Fuel Delivery', price: 600 },
  { value: 'battery', label: 'Jump Start / Battery', price: 1200 },
  { value: 'lockout', label: 'Lockout', price: 900 },
  { value: 'other', label: 'Other', price: 700 },
];
...
export function renderDriverDashboard(
  ...
  form.addEventListener('submit', async (e) => {
    ...
    toast('Help requested — searching for nearby mechanics.');
    await refreshList(listHost);
    pollUntilAccepted(listHost);
  });
  ...
  document.getElementById('use-gps')!.addEventListener('click', async () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        gpsBtn.disabled = false;
        gpsBtn.textContent = prev;
        toast('Could not get GPS location. Using mock location.', 'error');
      },
...
async function refreshList(host: HTMLElement): Promise<void> {
  ...
  if (requests.length === 0) {
    host.appendChild(emptyState('No requests yet. Request help to get started.'));
    return;
  }
...
function renderRequestCard(
  ...
  if (r.status === 'accepted' || r.status === 'in-progress') {
    ...
    enroute.innerHTML = `
      ...
      <span>${escapeHtml(r.mechanicName ?? 'A mechanic')} is en route.</span>
    `;
    card.appendChild(enroute);
  } else if (r.status === 'completed') {
    card.appendChild(el('p', 'muted', 'Job completed. Thank you!'));
  } else if (r.status === 'pending') {
    card.appendChild(el('p', 'muted', 'Searching for nearby mechanics…'));
  }
  return card;
}

let polling = false;
async function pollUntilAccepted(host: HTMLElement): Promise<void> {
  if (polling) return;
  polling = true;
  // Prototype: simulate a nearby mechanic accepting the newest pending
  // request a few seconds after it is created, so the driver sees the
  // "en route" flow without needing a second browser tab.
  setTimeout(async () => {
    polling = false;
    const res = await apiMyRequests();
    if (!res.ok) return;
    ...
    if (newest && newest.status === 'pending') {
      const ok = await simulateMechanicAccept(newest._id);
      if (ok) {
        await refreshList(host);
        toast('A mechanic accepted your request!');
      }
    }
  }, 4000);
}
